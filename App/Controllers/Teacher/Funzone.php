...
class Funzone
{
    ...
    public function index()
    {
        ...
        $this->view('Teacher/Funzone', !empty($media) ? ['media' => $media, 'result' => $result,"Profile" => $this->Profile()] : ['message' => 'No resource found']);
    }
    ...
    public function addMedia()
    {
        ...
        if (!$TeacherID) {
            ...
            $this->view('Teacher/Funzone', ['message' => 'Please log in to request a leave.', "Profile" => $this->Profile()]);
            return;
        }
            ...
            if (empty($result)) {
                ...
                if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
                    ...
                    if (!in_array($data['format'], $allowed_types)) {
                        $this->view('Teacher/Funzone', ['message' => 'Sorry, the file format in not allowed', "Profile" => $this->Profile()]);
                    } else {
                        ...
                        if (!($mediaModel->insert($arr))) {
                            ...
                            $this->view('Teacher/Funzone', ['message' => 'Failed to add resource. Please try again.', "Profile" => $this->Profile()]);
                        }
                    }
                } else {
                    $error['file'] = 'Please upload a file.';
                    $this->view('Teacher/Funzone', ['message' => $error, "Profile" => $this->Profile()]);
                }
            } else {
                // var_dump($result);
                // exit();
                $this->view('Teacher/Funzone', ['message' => $result, "Profile" => $this->Profile()]);
            }
        }
    }
    ...
    public function removeMedia()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($mediaModel->delete($id, 'MediaID')) {
                ...
                redirect('Teacher/Funzone', ['message' => 'Failed to remove resource. Please try again.',  "Profile" => $this->Profile()]);
            }
        } else {
    ...
    public function selectbyAge()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($selectedAgeGroup)) {
                ...
                //    exit();
                if (empty($media)) {
                    $this->view('Teacher/Funzone', ['message' => 'No Media Found',  "Profile" => $this->Profile()]);
                } else {
                    $this->view('Teacher/Funzone', ['media' => $media, 'result' => $result, "Profile" => $this->Profile()]);
                }
